...
ETResult apply_pitch_bend(WorldParameters* params,
    ...
    float* interpolated_bend = NULL;

    if (params->mem_pool) {
        interpolated_bend = (float*)et_alloc_from_pool(params->mem_pool,
                                                      params->f0_length * sizeof(float));
    } else {
        interpolated_bend = (float*)malloc(params->f0_length * sizeof(float));
    }
