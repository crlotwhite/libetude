...
ETResult et_create_thread_interface(ETThreadInterface** interface) {
    ...
    if (!factory->create_thread_interface) {
        ET_SET_ERROR(ET_ERROR_NOT_IMPLEMENTED, "스레딩 인터페이스 생성이 구현되지 않았습니다");
        return ET_ERROR_NOT_IMPLEMENTED;
    }

    return factory->create_thread_interface(interface);
}
...
void et_destroy_thread_interface(ETThreadInterface* interface) {
    ...
    const ETPlatformFactory* factory = et_platform_factory_get_current();
    if (factory && factory->destroy_thread_interface) {
